    ...
    <?php
      foreach($records as $roll => $data) {
        echo '<div class="student-record">';
         if($i<$num_row)
        echo'<button class="marker1">'.$n[$i].'</button><br/>'; 
        else
         echo'<button class="marker1"> Student</button><br>';
      $i++;

       echo ' <br>  <span class="roll"><a href="student.php?roll='.str_replace("/","-",$roll).'&code='.$node->getCode().'&year='.$node->getYear().'&section='.$node->getSection().'">'.$roll.'</a></span>: 
